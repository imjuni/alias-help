...
test('webpack-alias-from-tsconfig-sync', (t) => {
  const result = aliasHelpSync({ configFile: 'tsconfig.for.test.json', webpack5: true });
  const expect = {
    '@src': 'src',
...
test('webpack-alias-from-tsconfig-abs-sync', (t) => {
  ...
  const result = aliasHelpSync({ configFile: 'tsconfig.for.test.json', webpack5: true, useAbsolute: true });
  const expect = {
    '@src': path.join(cwd, 'src'),
    '@samsubdir': path.join(cwd, 'src/sample-sub-dir'),
    '@samdir': path.resolve(path.join(cwd, '../sample-dir')),
    '@avengers': [
      path.resolve(path.join(cwd, '../ironman')),
      path.resolve(path.join(cwd, '../hulk')),
      path.resolve(path.join(cwd, '../spiderman')),
    ],
  };
...
test('webpack-alias-from-tsconfig', async (t) => {
  const result = await aliasHelp({ configFile: 'tsconfig.for.test.json', webpack5: true });
  const expect = {
    '@src': 'src',
    '@samsubdir': 'src/sample-sub-dir',
    '@samdir': '../sample-dir',
...
test('webpack-alias-from-tsconfig-abs', async (t) => {
  const cwd = process.cwd();
  const result = await aliasHelp({ configFile: 'tsconfig.for.test.json', webpack5: true, useAbsolute: true });
  const expect = {
    '@src': path.join(cwd, 'src'),
    '@samsubdir': path.join(cwd, 'src/sample-sub-dir'),
    '@samdir': path.resolve(path.join(cwd, '../sample-dir')),
